...
ymax_all = 1.1 * max(max(Ndata));
   ...
   %Suzuki = 6 0
    ...
    % xticklabels(date(xticks))
    % xtickangle(45)
    % ymax = 1.1 * Nmax;
    % xlim([0 xtickR])
    % if projectionname(j) == "AB 3-3"
    %    ylim([0 0.3*Nmax])
    % else ylim([0 1.05*Nmax])
    % end
    ...
    if figure_save == 1
        ...
        file_name = strcat(projectionname(j), '_spaghetti_peakpm10.png');
...
fig_name_2 = "_spaghetti_by_phase_ma_peakpm10";
...
% actual_path_avg(:,1) = BTT(4).TT.actual(date_savg_2 + days(8)); 
% actual_path_avg(:,2) = BTT(4).TT.actual(date_savg_2 + days(15)); 
% actual_path_avg(:,3) = BTT(4).TT.actual(date_savg_2 + days(29)); 
...
fid = fopen('Table_peakpm10.txt', 'w');
